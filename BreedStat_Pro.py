...
with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
    ...
    top5_df.to_excel(writer, sheet_name="Top_5_Genotypes", index=False)
    recommend_df.to_excel(writer, sheet_name="Genotype_Recommendation", index=False)
...
if os.path.exists(blup_stability_plot):
    ...
    ws = wb.create_sheet("BLUP_Stability")

    img = XLImage(blup_stability_plot)
    img.anchor = "A1"
...
if os.path.exists(regression_plot_file):
    ...
    ws = wb.create_sheet("Regression_Test_vs_Check")

    img = XLImage(regression_plot_file)
    img.anchor = "A1"
...
if os.path.exists(gge_plot_file):
    ...
    ws = wb.create_sheet("GGE_Biplot")

    img = XLImage(gge_plot_file)
    img.anchor = "A1"
...
if os.path.exists(ammi1_plot):
    ...
    ws = wb.create_sheet("AMMI1_Biplot")

    img = XLImage(ammi1_plot)
    img.anchor = "A1"
...
if os.path.exists(ammi2_plot):
    ...
    ws = wb.create_sheet("AMMI2_Biplot")

    img = XLImage(ammi2_plot)
    img.anchor = "A1"
...
if os.path.exists(gge_polygon_plot):
    ...
    ws = wb.create_sheet("GGE_WhichWonWhere")

    img = XLImage(gge_polygon_plot)
    img.anchor = "A1"
